...
const heartbeatSchema = z.object({
  ...
  prompt: z.string().default('Check if anything needs attention and report using the structured heartbeat format.'),
...
function shouldAutoActivateOllama(config: AIProviderConfig): boolean {
  const autoActivate = process.env.OLLAMA_AUTO_ACTIVATE
  if (autoActivate === 'true') return true
  if (autoActivate === 'false') return false
  return !config.profiles[config.activeProfile]
}

/** @ai-context Apply OLLAMA_BASE_URL / OLLAMA_MODEL env overrides to AI provider config without clobbering an explicit active profile. */
function applyOllamaEnvOverride(config: AIProviderConfig): AIProviderConfig {
  ...
  return {
    ...
    activeProfile: shouldAutoActivateOllama(config) ? 'ollama' : config.activeProfile,
  }
}

export async function loadConfig(): Promise<Config> {
  const files = ['engine.json', 'agent.json', 'crypto.json', 'securities.json', 'market-data.json', 'compaction.json', 'ai-provider-manager.json', 'heartbeat.json', 'snapshot.json', 'connectors.json', 'news.json', 'tools.json', 'trading.json', 'webhook.json'] as const
