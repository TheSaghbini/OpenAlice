...
- **Multi-provider AI** — Claude (Agent SDK with OAuth or API key), Codex, or Vercel AI SDK profiles (Anthropic, OpenAI, Google, and OpenAI-compatible endpoints such as Ollama), switchable at runtime
...
**Providers** — interchangeable AI backends. Claude (Agent SDK) uses `@anthropic-ai/claude-agent-sdk` with tools delivered via in-process MCP. Codex uses OpenAI subscription OAuth or API key flows. Vercel AI SDK handles direct API calls to Anthropic, OpenAI, Google, and OpenAI-compatible endpoints such as Ollama. `ProviderRouter` reads `ai-provider-manager.json` on each call to select the active backend at runtime.
...
**AI Provider** — The AI backend that powers Alice. Claude (via Agent SDK, supports OAuth login or API key), Codex, or Vercel AI SDK (Anthropic, OpenAI, Google, and OpenAI-compatible endpoints such as Ollama). Switchable at runtime — no restart needed.

## Quick Start

Prerequisites: Node.js 22+, pnpm 10+. [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) is only required if you want to use the default Claude profile.
...
Open [localhost:3002](http://localhost:3002) and start chatting. The default setup uses your local Claude Code login (Claude Pro/Max subscription). If you want local models instead, start Ollama and create the built-in Ollama profile from the AI Provider page.
...
**AI Provider** — The default provider is Claude (Agent SDK), which uses your local Claude Code login — no API key needed. To use the [Vercel AI SDK](https://sdk.vercel.ai/docs) instead, create a profile in `ai-provider-manager.json` or from the Web UI. Built-in presets include Claude (Subscription), Claude (API Key), OpenAI Codex (Subscription), OpenAI (API Key), Google Gemini, Ollama, and MiniMax. The Custom preset is for anything outside those defaults, including Anthropic-compatible or other OpenAI-compatible endpoints. Ollama defaults to `http://localhost:11434/v1`, and its API key field is optional for local deployments. If `OLLAMA_BASE_URL` is set, OpenAlice injects or refreshes an `ollama` profile from that environment value without overriding your current active profile; set `OLLAMA_AUTO_ACTIVATE=true` if you want the env-managed Ollama profile to become the active default automatically.
...
| `ai-provider-manager.json` | AI provider profiles, active profile selection, and per-profile auth/settings switchable at runtime |
